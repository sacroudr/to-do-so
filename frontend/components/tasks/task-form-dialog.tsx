 ...
 * statut, priorite, source. Soumission via Server Action (create/update). Le retour
 * (succes / erreur) passe par la notification globale (§8), plus de message inline.
 * Champs et boutons mutualises via les primitives partagees (§2 uniformite).
 */
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>): void {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        toast.success(
          mode === "create"
            ? `Tâche « ${values.titre} » créée.`
            : `Tâche « ${values.titre} » modifiée.`,
        );
        onClose();
      } else {
